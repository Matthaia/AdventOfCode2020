var input = `wavy turquoise bags contain no other bags.
...
function GetBag(bagName) 
{
    return bags.find(x => x.Name == bagName);
};
...
class Bag {
    ...
    AddBag(nameString)
    {
        if(nameString.includes("no other bags"))
            return;

        var firstSpace = nameString.indexOf(" ");
        var parts = [nameString.slice(0, firstSpace), nameString.slice(firstSpace + 1)];

        var amount = parseInt(parts[0]);
        var bagName = parts[1][parts[1].length - 1] == "s" ? parts[1].substring(0, parts[1].length - 1) : parts[1];

        this.Contains.set(bagName, amount);
    }

    get ChildBags() {
        return [...this.Contains.keys()].map(x => GetBag(x));
    }

    ContainsShinyGoldBag() 
    {
        if([...this.Contains.keys()].find(x => x.includes("shiny gold")))
            return true;

        return this.ChildBags.map(x => x.ContainsShinyGoldBag()).find(x => x)
    }
    ...
    AmountOfBags() 
    {
        ...
        for(var i = 0; i < this.Contains.size; i++)
        {
            var key = [...this.Contains.keys()][i];
            var value = [...this.Contains.values()][i];
            result += value * GetBag(key).AmountOfBags();
            result += value;
        }
...
var bags = input.map(x => {
    var parts = x.split("contain");

    var name = parts[0].trim().slice(0, -1); // Remove the trailing s with slice.

    var bag = new Bag(name);

    parts[1].split(", ").forEach(y => {
        bag.AddBag(y.trim().replace(".", ""));
    });

    return bag;
});

var hasShinyGoldBag = bags.map(x => x.ContainsShinyGoldBag()).filter(x => x);
console.log("Exercise 1: " + hasShinyGoldBag.length);

var shinyGoldBag = GetBag("shiny gold bag");

console.log("Exercise 2: " + shinyGoldBag.AmountOfBags());