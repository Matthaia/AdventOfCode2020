...
class Group {
    ...
    get CorrectAnswers() {
        var answers = this.persons.map(x => x.correctAnswers).flatMap(x => x)

        return answers.filter((x, index, self) => self.indexOf(x) === index)
    }
    ...
    get AllAnsweredYes() {
        ...
        for(var i = 0; i < letters.length; i++)
        {
            ...
            {
                var person = this.persons[j];
                if(!person.HasAnswer(letter))
                {
                    found = false;
                    break;
                }
...
class Person {
    constructor(order, correctAnswers){
        this.order = order;
        this.correctAnswers = correctAnswers;
    }

    HasAnswer(letter) {
        return this.correctAnswers.includes(letter);
    }
}

var groups = input.map(x => {
    var lines = x.split("\n");

    var persons = lines.map((line, index) => new Person(index, line.trim().split("")));

    return new Group(persons);
});

var exercise1 = groups.map(x => x.CorrectAnswers).reduce((prev, value) => value.length + prev, 0);

console.log("Exercise 1: " + exercise1);

var exercise2 = groups.map(x => x.AllAnsweredYes).reduce((prev, value) => value.length + prev, 0);
