...
var valid = input.map(x => {
    var birthYear = parseInt(x.get("byr") || "0");
    if (birthYear < 1920 || birthYear > 2002)
        return;

    var issueYear = parseInt(x.get("iyr") || "0");
    if (issueYear < 2010 || issueYear > 2020)
        return;

    var expirationYear = parseInt(x.get("eyr") || "0");
    if (expirationYear < 2020 || expirationYear > 2030)
        return;

    var heigth = x.get("hgt") || "0";
    if(heigth.includes("cm"))
    {
        var heigthInCm = parseInt(heigth.replace("cm", ""))
        if(heigthInCm < 150 || heigthInCm > 193)
            return;
    }
    else if (heigth.includes("in"))
    {
        var heigthInInch = parseInt(heigth.replace("in", ""))
        if(heigthInInch < 59 || heigthInInch > 76)
            return;
    }
    else 
    {
    ...
    var hairColor = x.get("hcl") || "";
    if(hairColor.substring(0, 1) != "#" || hairColor.length != 7)
        return;

    var eyeColor = x.get("ecl") || "";
    if(!["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].includes(eyeColor)) 
        return;

    var passportID = x.get("pid") || "";
